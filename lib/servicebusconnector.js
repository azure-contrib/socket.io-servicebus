...
ServiceBusConnector.prototype.start = function (startedCallback) {
  ...
  self.ensureTopicAndSubscriptionExist(function (err) {
    if (!err) {
      ...
      self.log && self.log.error('Could not set up Service Bus', 'err:' + err.toString());
    }
...
function logCreateResultAndReturnError(log, fullName, shortName, name, err) {
    if (err && +err.code !== 409) {
      log && log.error('Error creating ' + fullName, shortName + ':' + name, 'error:' + err.toString());
      return err;
    } else if (err && +err.code === 409) {
      log && log.info('Using existing ' + fullName, shortName + ':' + name);
      return null;
    } else {
      log && log.info('Created ' + fullName, shortName + ':' + name);
      return null;
    }
}
