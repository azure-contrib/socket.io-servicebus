...
ServiceBusConnector.prototype.start = function (startedCallback) {
  ...
  self.ensureTopicExists(function (err) {
    if (!err) {
      self.ensureSubscriptionExists(function(err) {
        if (!err) {
          ...
          self.log && self.log.error('Subscription creation failed', 'err:' + err.toString());
        }
        startedCallback && startedCallback(err);
      });
    } else {
      self.log && self.log.error('Topic creation failed', 'err:' + err.toString());
      startedCallback && startedCallback(err);
    }
  });
};
...
ServiceBusConnector.prototype.ensureTopicExists = function(callback) {
  ...
  this.serviceBusSender.createTopic(this.topic, topicOptions, function (err, result, response) {
    if (err && +err.code !== 409) {
      // code 409 means topic already exists, any other code is real error
      self.log && self.log.error('Error creating topic', 'topic:' + self.topic, 'error:' + err.toString());
    } else if (err && +err.code === 409) {
      self.log && self.log.info('Using existing topic', 'topic:' + self.topic);
      err = null;
    } else {
      self.log && self.log.info('Topic created', 'topic:' + self.topic);
    }
    callback(err);
  });
};
...
ServiceBusConnector.prototype.ensureSubscriptionExists = function(callback) {
  ...
  self.serviceBusSender.createSubscription(self.topic, self.subscription, subscriptionOptions, function(err){
    if (err && +err.code !== 409) {
      self.log && self.log.error('Error creating subscription', 'sub:' + self.subscription, 'error:' + err.toString());
    } else if (err && +err.code === 409) {
      self.log && self.log.info('Subscription already exists', 'sub:' + self.subscription);
      err = null;
    } else {
      self.log && self.log.info('Subscription created', 'sub:' + self.subscription);
    }
    callback(err);
  });
};
