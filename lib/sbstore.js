...
function ServiceBusStore(options) {
  ...
  this.formatter = options && options.messageFormatter;
  this.sb = options && options.serviceBusInterface;
  this.subscribers = {};
}
...
ServiceBusStore.prototype.publish = function (name) {
  ...
  this.sb.send(name, this.formatter.pack(args));
...
ServiceBusStore.prototype.receiveMessage = function (sbMessage) {
  ...
  if (message.nodeId !== this.nodeId) {
    log('Store nodeId %s sending message to subscribers');
