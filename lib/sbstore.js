...
function ServiceBusStore(options) {
  ...
  this.formatter = this.createFormatter(this.nodeId);
...
ServiceBusStore.prototype.publish = function (name) {
  ...
  this.sb.send(this.formatter.pack(name, args));
...
ServiceBusStore.prototype.receiveMessage = function (sbMessage) {
  ...
  this.emit.apply(this, ['received', message.name].concat(message.args));
...
ServiceBusStore.prototype.createServiceBusInterface = function createServiceBusInterface(options) {
  ...
  return new ServiceBusInterface(serviceBusService, options.topic, options.subscription);
}

ServiceBusStore.prototype.createFormatter = function (nodeId) {
 ...
 *    onreceived(name) - message has been received, varargs for rest
