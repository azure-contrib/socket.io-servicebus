...
function ServiceBusStore(options) {
  ...
  this.formatter = (options && options.messageFormatter) || new Formatter(this.nodeId);
...
ServiceBusStore.prototype.createServiceBusInterface = function createServiceBusInterface(options) {
  log('Creating real service bus interface');
  this.sb = new serviceBusInterface(options.serviceBusService, options.topic, options.subscription);
}
